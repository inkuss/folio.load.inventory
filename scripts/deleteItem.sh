...
# Hilfe: deleteItem.sh -h
source shell_functions.sh

# Default-Parameter-Werte
useFile=0
folio_json_file="../sample_inventory/items/4711.json"
...
usage() {
  ...
  1. Aufruf ohne Optionen : Löscht ein Exemplar anhand einer Item-ID.
     Aufruf:                ./deleteItem.sh itemId -u myokapiurl
     benötigt: login.json im gleichen Verzeichnis.
     Beispielaufruf:        ./deleteItem.sh 9ec4aea2-b4f7-5ab0-bf54-87a66f135bbd -u myokapiurl
  2. Aufruf mit Parameteroption -f : Löscht ein Exemplar anahnd einer Datei im Format FOLIO-JSON. Parst Item-ID aus Datei.
     Beispielaufruf:        ./deleteItem.sh -f ../sample_inventory/items/4711.json -u myokapiurl

  Optionen:
   - f [Datei]      Item-ID wird aus Datei gelesen, Standardwert: $folio_json_file
   - h              Hilfe (dieser Text)
...
curl $curlopts -S -X DELETE -H "$TOKEN" -H "X-Okapi-Tenant: $tenant" -H "Content-type: application/json; charset=utf-8" $okapi_url/item-storage/items/$id
echo
