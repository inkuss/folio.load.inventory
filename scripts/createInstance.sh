...
# Hilfe: createInstance.sh -h
source shell_functions.sh

# Default-Parameter-Werte
folio_json_datei="../sample_inventory/instances/1890.json"
...
usage() {
  ...
  Beispielaufruf:        ./createInstance.sh -f ../sample_inventory/instances/1890.json -u myokapiurl -t mytenant

  Optionen:
   - f [Datei]      Titeldaten im Format FOLIO-JSON, Standardwert: $folio_json_datei
   - h              Hilfe (dieser Text)
...
curl $curlopts -S -X POST -H "$TOKEN" -H "X-Okapi-Tenant: $tenant" -H "Content-type: application/json; charset=utf-8" -H "Accept: application/json" -d \@$folio_json_datei $okapi_url/inventory/instances
echo
