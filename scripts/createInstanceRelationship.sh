...
# Hilfe: createInstanceRelationship.sh -h
source shell_functions.sh

# Default-Werte der Skript-Parameter
folio_json_datei="../sample_inventory/instanceRelationships/1891_IR1.json"
...
usage() {
  ...
  Beispielaufruf:  ./createInstanceRelationship.sh -f ../sample_inventory/instanceRelationships/1891_IR1.json -u myokapiurl

  Optionen:
   - f [Datei]      Instanzbeziehung im Format FOLIO-JSON, Standardwert: $folio_json_datei
   - h              Hilfe (dieser Text)
...
curl $curlopts -S -X POST -H "$TOKEN" -H "X-Okapi-Tenant: $tenant" -H "Content-type: application/json; charset=utf-8" -H "Accept: application/json" -d \@$folio_json_datei $okapi_url/instance-storage/instance-relationships
echo
